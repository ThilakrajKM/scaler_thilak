...
public class TowersOfHanoi {
    ...
    public static void toh(int disk, int source, int help, int dest, ArrayList<ArrayList<Integer>> result) {
        if (disk == 0) {
            return;
        }
        //moving disk-1 from source to help
        toh(disk - 1, source, dest, help, result);

        //disk is moved to source to destination
        ArrayList<Integer> resPart = new ArrayList<>();
        resPart.add(disk);
        resPart.add(source);
        resPart.add(dest);
        result.add(resPart);

        //moving disk-1 help to destination
        toh(disk - 1, help, source, dest, result);
    }
